...
Unemployed_individuals <- (Population * .2) * Unemployment_Rate
Unemployed_individuals
```
...
```{r simple-var-practice-4, eval=FALSE}
class(Country)
class(GDP)
...
# Define vector of zeros with length 5 zero 5 times
rep(0,5)

# We can combine multiple vectors with c() c stand for cobine
x<-c(1:10,rep(0,5))
x
...
Min_Q_GDP_growth_ratem = min(Q_GDP_growth_rate)
Min_Q_GDP_growth_ratem
...
library(probstats4econ)
cps%>%glimpse()
cps%>%head()#frist 5 rows of the data to give you a glimps
```
